    ...
    <Provider template={AlertTemplate} {...options}>
      <div className="block md:flex md:h-screen font-sans min-h-screen bg-arian-negro">
        <Head>
          ...
          <meta
            name="description"
            content="Siendo Web Developer, ¿por qué tendría un curriculum en PDF?"
          />
        ...
        <div className="block md:w-1/2 md:h-full lg:w-auto lg:flex">
          ...
          <LeftColumn
            profile={{ name, lastName, occupation, avatar }}
            skills={getSkills}
          />
        </div>
...
export async function getServerSideProps() {
  const readAll = Promise.all([Skill.find({}), Profile.find({})])
    .then((values) => {
